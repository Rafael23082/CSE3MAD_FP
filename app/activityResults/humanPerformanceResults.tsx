...
export default function HumanPerformanceResultsScreen() {
  const { theme } = useTheme();
  ...
  useEffect(() => {
    AsyncStorage.getItem(RATING_KEY + 'stretch-speed-and-gracefulness').then(val => { if (val) setRating(parseInt(val, 10)); });
  }, []);
    ...
    (async () => {
      try {
        for (const log of logs) {
          await saveExperimentLog('stretch-speed-and-gracefulness', log.data, log.timestamp);
        }
      } catch (e) {
  ...
  const saveRating = useCallback((n: number) => {
    setRating(n);
    AsyncStorage.setItem(RATING_KEY + 'stretch-speed-and-gracefulness', String(n));
    sqliteSaveRating('stretch-speed-and-gracefulness', n).catch(() => {});
  }, []);
    ...
    >
      ...
      <Text style={styles.subtitle}>{t("results.attempts")}</Text>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.theoryText}>{t("results.movementTheory")}</Text>
        </View>
...
const createStyles = (colors: any) => StyleSheet.create({
  ...
  smoothnessValue: { fontSize: 14, fontWeight: "bold", color: colors.tertiary, marginBottom: 2 },
